...
from app.models.projects import Course, Student, Major
...
def init_database(request,test_client):
    ...
    course1 = Course(major_id = cs_major.id, coursenum =  '3733', title = "Software Engineering")
...
def test_register(request,test_client,init_database):
    ...
    assert b"Welcome!" in response.data  #Students should update this assertion condition according to their own page content
...
def do_login(test_client, path , username, passwd):
    response = test_client.post(path, 
                          ...
                          follow_redirects = True)
    assert response.status_code == 200
    assert b"Welcome back!" in response.data  #Students should update this assertion condition according to their own page content
...
def test_create_course(request,test_client,init_database):
    ...
    c = db.session.scalars(sqla.select(Course).where(Course.coursenum == '1101').where(Course.major_id == cs_major.id)).first()
    c_count = db.session.scalar(sqla.select(db.func.count()).where(Course.coursenum == '1101').where(Course.major_id == cs_major.id))
...
def test_enroll(request,test_client,init_database):
    ...
    cs_major = db.session.scalars(sqla.select(Major).where(Major.name == 'CS')).first()
    course1 = db.session.scalars(sqla.select(Course).where(Course.coursenum == '3733').where(Course.major_id == cs_major.id)).first()
    ...
    response = test_client.post('course/'+ str(course1.id) + '/enroll', 
                        ...
                        follow_redirects = True)
    assert response.status_code == 200
    assert b"Successfully enrolled in course CS 3733" in response.data #Students should update this assertion condition according to their own page content
    c = db.session.query(Course).filter(Course.coursenum =='3733' and Course.major_id == cs_major.id).first()
    thecourse = db.session.scalars(sqla.select(Course).where(Course.coursenum == '3733').where(Course.major_id == cs_major.id)).first()
